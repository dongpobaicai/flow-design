...
type emptyProps = {} | null;

export default function getEmptyConfig(design: FlowDesign, props?: emptyProps): NodeConfig {
  return {
    name: "emptyNode",
    entity: {
        ...
        addApprovalNode({ key, graph, node }: { key: string; graph: Graph; node: Node }) {
          ...
          if (key == "ConditionNode") {
            ...
            if (lastNode[0].component == "conditionNode") {
              ...
              let emptyNodeItance = createNode("EmptyNode", graph);
              cells = [
                emptyNodeItance,
                member,
                member2,
